...
occupancylib = np.ctypeslib.load_library("libdescriptor_ext", "./descriptor/")

thread_number = 8
image_size = 48
half_image_length = image_size/2.0
image_resolution = 0.5
save_folder='./image/temp_48_small_partial_charge_radius_with_H/'
...
#########################################################################################
datfile = sys.argv[1]
print('usage: python 2-voxelization.py datfilenum')

PDBSetList=[]
with open('./MG_Res3.0/selected-MG-pdb-'+datfile+'.dat') as PDBFile:
    for line in PDBFile:
        if line[0]!='#':
            record=line.split()
            PDBname=record[0]
            PDBSetList.append(PDBname)
print(len(PDBSetList))
...
N1baseList = ["C","U"]
N9baseList = ["A","G"]
for PDBname in PDBSetList:
    print(PDBname)
    rna_structure = parser.get_structure(PDBname, base_folder+PDBname+'/'+PDBname+'_RNA.pdb')
    whole_structure_path = base_folder+PDBname+'/'+PDBname+'_original.pdbqt'
    ...
    for model in [rna_models[0]]:
        ...
        for chain in model:
            ...
            for residue in chain:
                ...
                save_path = save_folder+PDBname+'/'+PDBname+'_'+str(chain_count)+'_'+str(residue_count)+'_'+residue_name+'.npy'
                C1Prime=None
                O4Prime=None
                NitrogenBase=None
                for atom in residue:
