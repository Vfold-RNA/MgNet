...
print('usage: python cluster.py input_folder output_folder')
...
#parser = PDBParser()
file_list = dataset_list(input_folder)
...
for preFile in file_list:
    ...
    clean_expt_structure = parser.get_structure(pdbname, '../../02-prepare-for-cnn/processed/'+pdbname+'_RNA.pdb')
    num_res = 0
    clean_expt_models = []
    ...
    expt_structure = parser.get_structure(pdbname, '../../02-prepare-for-cnn/processed/'+pdbname+'.pdb')
    old_wrong_num_res = 0
    expt_models = []
    expt_MGs = []
    ...
    with open(output_folder+pdbname+'_cv'+cv_num+'_cluster.pdb','w') as f:
        serial = 0
        for h in hits:
            serial+=1
