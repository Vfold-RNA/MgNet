...
for preFile in file_list:
    pdbname = preFile.strip().split("/")[-1][0:4]
    print(pdbname)

    clean_expt_structure = parser.get_structure(pdbname, '../../data/processed/'+pdbname+'/'+pdbname+'_RNA.pdb')
    num_res = 0
    clean_expt_models = []
    ...
    expt_structure = parser.get_structure(pdbname, '../../data/processed/'+pdbname+'/'+pdbname+'_original.pdb')
    old_wrong_num_res = 0
    expt_models = []
    expt_MGs = []
